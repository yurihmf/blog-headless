  ...
  return (
        ...
        <article key={post.node.slug} className="flex">
          <div className="relative w-48 h-36 p-2 mr-4">
            ...
            <span className="text-white relative z-10 text-sm bg-purple-light px-4 max-w-max rounded-sm">{post.node.categories.map(category => category.title)}</span>
          </div>
          <div className="flex flex-col gap-y-2.5 w-3/5">
              ...
              <Link href={post.node.slug}>
                <p className=" text-white font-inter font-medium text-sm bg-purple-light px-4 py-1 max-w-max rounded-sm cursor-pointer">Ler notícias</p>
              </Link>
          </div>
        </article>
        ))
